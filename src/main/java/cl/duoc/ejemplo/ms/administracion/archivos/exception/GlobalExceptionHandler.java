...
public class GlobalExceptionHandler {
	...
	/**
	 * Maneja excepciones de guía de despacho no encontrada
	 */
	@ExceptionHandler(GuiaNotFoundException.class)
	public ResponseEntity<ErrorResponse> handleGuiaNotFoundException(GuiaNotFoundException ex, WebRequest request) {

		log.error("Guía no encontrada: {}", ex.getMessage());

		ErrorResponse error =
				ErrorResponse.builder().timestamp(LocalDateTime.now()).status(HttpStatus.NOT_FOUND.value())
						.error("Guia Not Found").message(ex.getMessage()).path(getPath(request)).build();

		return new ResponseEntity<>(error, HttpStatus.NOT_FOUND);
	}

	/**
	 * Maneja errores de validación de los request body
	 */
	...
	public ResponseEntity<ErrorResponse> handleValidationException(MethodArgumentNotValidException ex,
		...
		ErrorResponse error =
				ErrorResponse.builder().timestamp(LocalDateTime.now()).status(HttpStatus.BAD_REQUEST.value())
						.error("Validation Error").message("Los datos enviados no son válidos").details(detalle)
						.path(getPath(request)).build();
